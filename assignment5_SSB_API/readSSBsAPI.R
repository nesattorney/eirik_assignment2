...
dframetidy <- dframe %>% mutate(region = dplyr::recode(region,
                                                      "Tr�ndelag - Tr��ndelage"="Tr�ndelag",
                                                      "Troms og Finnmark - Romsa ja Finnm�rku" = "Troms og Finnmark"))

room_capacity <- dframetidy[dframetidy$variable1 == "roomcap", ]

ggplot(room_capacity, aes(x=month, y=value, group=region)) +
  geom_line(aes(color=region)) +
  ggtitle("Room Capacity")
